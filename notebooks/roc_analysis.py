...
def get_convex_hull(graph_x, graph_y):
...
def get_roc_graph_linear(
    pos_x : th.Optional[np.ndarray] = None,
    pos_y : th.Optional[np.ndarray] = None,
    neg_x : th.Optional[np.ndarray] = None,
    neg_y : th.Optional[np.ndarray] = None,
    compute_limit: int = 100,
    verbose: int = 1,
):
    if pos_x is None or neg_x is None:
        raise Exception("pos_x and neg_x should be defined")
    ...
    pos_x = (pos_x - np.mean(all_x)) / (1.0 if np.std(all_x) < 1e-3 else np.std(all_x))
    neg_x = (neg_x - np.mean(all_x)) / (1.0 if np.std(all_x) < 1e-3 else np.std(all_x))
    pos_y = (pos_y - np.mean(all_y)) / (1.0 if np.std(all_y) < 1e-3 else np.std(all_y))
    neg_y = (neg_y - np.mean(all_y)) / (1.0 if np.std(all_y) < 1e-3 else np.std(all_y))
    
    rng = np.linspace(-np.pi/2 + 1e-3, np.pi/2 - 1e-3, compute_limit)
    if verbose > 0:
        rng = tqdm(rng)
    ...
    for deg in rng:
        slope = np.tan(deg)
        pos_scores = pos_y - slope * pos_x
        neg_scores = neg_y - slope * neg_x
        graph_x, graph_y = get_roc_graph(
            pos_x=pos_scores,
            neg_x=neg_scores, 
            verbose=0,
        )
        points += [(x, y) for x, y in zip(graph_x, graph_y)]
